...
# File reading buffer (Reduced: 10 nums * ~12 chars max = ~120 bytes)
file_content:   .space 256        

# Signal arrays (Reduced: 10 floats * 4 bytes = 40 bytes)
.align 2
input:          .space 40          # EXACTLY 10 floats
desired:        .space 40          # EXACTLY 10 floats
output:         .space 40          # EXACTLY 10 floats
...
# =========================================================================
# FILE READING SUBROUTINE (LIMIT 10)
# =========================================================================
read_file_to_array:
    addi $sp, $sp, -20
    sw $ra, 0($sp)
    ...
    move $a0, $s1
    la $a1, file_content
    li $a2, 256                 # Matching new smaller buffer size
    syscall
...
rounding_step:
    ...
    # === NEW: LIMIT CHECK ===
    li $t9, 10                  # Limit to 10 numbers
    beq $s3, $t9, done_parse    # If 10 read, exit function immediately
    # ========================
...
process_last:
    # Process last number if file ends without newline
    mtc1 $t3, $f0
    cvt.s.w $f0, $f0
...
PrintOutputLoop:
    ...
    # Load and round to 1 decimal
    sll $t2, $t0, 2
    la $t3, output
    add $t3, $t3, $t2
    l.s $f12, 0($t3)
    
    # Round to 1 decimal place (WITH STACK SAVE)
    addi $sp, $sp, -8
    sw $ra, 0($sp)
    sw $t0, 4($sp)         # Save $t0
    ...
    # Print space (for clarity in console)
    li $v0, 4
    la $a0, space
    syscall
...
float_to_string:
    ...
    sb $t1, 0($s7)
    addi $s7, $s7, 1
    # FIXED: Deleted the duplicate 'sb' here that caused "--"
    
    # NO Null terminate here, continue to print number
